...
#
def create_geometry_file_upload(geometry_path)
...
# @return [Hash] Hash containing results of the GraphQL request
#
def create_file_uploads(file_paths)
...
# @return [Hash] Hash containing results of the GraphQL request
#
def import_feature_set(survey_sentera_id:, geometry_file_upload:, file_uploads:)
  ...
  variables = {
    owner_type: 'SURVEY',
    owner_sentera_id: survey_sentera_id,
    name: 'My Feature Set',
    type: 'UNKNOWN',
...
geometry_file_upload = create_geometry_file_upload(geometry_path)
...
file_uploads = create_file_uploads(file_paths)
if file_uploads.nil?
  puts 'Failed'
  exit
end
...
if results
  puts "Done! Feature set with #{file_uploads.size} files was queued for importing."
else
  puts 'Failed'
end
